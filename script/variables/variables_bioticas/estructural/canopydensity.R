...
# 3. Cálculo de los percentiles de la altura básicos a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  ...
  canopydensity <- lidR::pixel_metrics(
    teselas_n,
    func = ~lidRmetrics::metrics_canopydensity(
      z = Z, 
      interval_count = 10,
      zmin = 0.5
    ),
  ...
  terra::writeRaster(
    canopydensity, 
    here::here(paste0("data/variables/variables_bioticas/canopydensity/canopydensity_", conjunto, ".tif"))
  )
  
  # Limpieza de la memoria
  rm(canopydensity, teselas_n)
  gc(full = TRUE)
