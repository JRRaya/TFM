...
# 3. Cálculo de los percabove de la altura a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  ...
  percabove <- lidR::pixel_metrics(
    teselas_n,
    func = ~lidRmetrics::metrics_percabove(
      z = Z, 
      threshold = c(0.5, 2.5, 4),
      zmin = 0.5
    ),
    res = 10
  ) %>% 
  ...
  terra::writeRaster(
    percabove, 
    here::here(paste0("data/variables/variables_bioticas/percabove/percabove_", conjunto, ".tif"))
  )
  
  # Limpieza de la memoria
  rm(percabove, teselas_n)
  gc(full = TRUE)
