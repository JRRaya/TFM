...
# 3. Cálculo de los percentiles de la altura a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  ...
  percentiles <- lidR::pixel_metrics(
    teselas_n,
    func = ~lidRmetrics::metrics_percentiles(
      z = Z, 
      zmin = 0.5
    ),
    res = 10
  ...
  terra::writeRaster(
    percentiles, 
    here::here(paste0("data/variables/variables_bioticas/percentiles/percentiles_", conjunto, ".tif"))
  )
  
  # Limpieza de la memoria
  rm(percentiles, teselas_n)
  gc(full = TRUE)
