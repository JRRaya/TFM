...
# 3. Cálculo de las métricas calculadas en un spacio 'voxel' a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  ...
  voxels <- lidR::pixel_metrics(
    teselas_n,
    func = ~lidRmetrics::metrics_voxels(
      x = X, 
      y = Y,
      z = Z, 
      vox_size = 1.5,
  ...
  terra::writeRaster(
    voxels, 
    here::here(paste0("data/variables/variables_bioticas/voxels/voxels_", conjunto, ".tif"))
  )
