...
# 3. Crear función de cálculo de la dimensión fractal
fd = function(X,Y,Z) {
  M = cbind(X,Y,Z)
  Rdimtools::est.boxcount(M)$estdim
}

# 4. Cálculo de los percentiles de la altura básicos a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  ...
  terra::writeRaster(
    fd, 
    here::here(paste0("data/variables/variables_bioticas/fd/fd_", conjunto, ".tif"))
  )
