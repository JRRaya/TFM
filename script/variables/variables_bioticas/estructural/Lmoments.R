...
# 3. Cálculo de los Lmoments de la altura a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  ...
  Lmoments <- lidR::pixel_metrics(
    teselas_n,
    func = ~lidRmetrics::metrics_Lmoments(
      z = Z, 
      zmin = 0.5
    ),
    res = 10
  ...
  terra::writeRaster(
    Lmoments, 
    here::here(paste0("data/variables/variables_bioticas/Lmoments/Lmoments_", conjunto, ".tif"))
  )
  
  # Limpieza de la memoria
  rm(Lmoments, teselas_n)
  gc(full = TRUE)
