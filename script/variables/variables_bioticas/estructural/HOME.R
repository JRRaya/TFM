...
# 3. Cálculo de los HOME de la altura a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  # Carga de teselas 
  teselas_n <- lidR::readLAScatalog(
    folder = list.files(
      path = file.path("data/variables/lidar", "lidar1", "norm"),
      pattern = "\\.laz$",
      full.names = TRUE
    ), 
  ...
  HOME <- lidR::pixel_metrics(
    teselas_n,
    func = ~lidRmetrics::metrics_HOME(
      z = Z, 
      i = i,
      zmin = 0.5
    ),
  ...
  terra::writeRaster(
    HOME, 
    here::here(paste0("data/variables/variables_bioticas/HOME/HOME_", conjunto, ".tif"))
  )
