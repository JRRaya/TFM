...
# 3. Cálculo de la estructura vertical a nivel de píxel para cada uno de los conjuntos de datos LiDAR
for (conjunto in conjuntos_lidar) {
  ...
  dispersion <- lidR::pixel_metrics(
    teselas_n,
    func = ~lidRmetrics::metrics_dispersion(
      z = Z, 
      dz = 1, 
      zmin = 0.5,
      zmax = 30
  ...
  terra::writeRaster(
    dispersion, 
    here::here(paste0("data/variables/variables_bioticas/dispersion/dispersion_", conjunto, ".tif"))
  )
  
  # Limpieza de la memoria
  rm(dispersion, teselas_n)
  gc(full = TRUE)
